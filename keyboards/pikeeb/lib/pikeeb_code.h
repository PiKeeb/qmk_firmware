...
static uint32_t VBAT_TIME = 60000; // time in ms to take the VBAT voltage measurement
static uint32_t RPI_TIME = 120000; // time in ms to take the RPI voltage measurement
static uint32_t INACTIVE_TIME_LOGO = 30000; // time in ms to show the logo
static uint32_t INACTIVE_TIME_OFF = 600000; // time in ms to turn off the oled
...
void switch_ts(void) {
    ...
    eeconfig_update_user(user_config.raw);
    if (user_config.touchscreen_on) { // ON = PC MODE
...
void matrix_scan_user(void) {
   ...
   if (timer_elapsed32(RPIV_TIMER) > RPI_TIME) {
       rpiv_measure();
       RPIV_TIMER = timer_read32();
   }
}
